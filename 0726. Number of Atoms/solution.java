 ...
 * 
 * Write a function parse that parses the formula from index i, returning a map count from names to multiplicities
 * (the number of times that name is recorded).
 * 
 * We will put i in global state: our parse function increments i throughout any future calls to parse.
 * 
 * When we see a '(', we will parse whatever is inside the brackets (up to the closing ending bracket) and add it
 * to our count.
 * 
 * Otherwise, we should see an uppercase character: we will parse the rest of the letters to get the name, and add
 * that (plus the multiplicity if there is one.)
 * 
 * At the end, if there is a final multiplicity (representing the multiplicity of a bracketed sequence), we'll
 * multiply our answer by this.
 *
 * Time Complexity: O(N^2), where N is the length of the formula. It is O(N) to parse through the formula, but each of O(N) multiplicities after a bracket may
 *                  increment the count of each name in the formula (inside those brackets), leading to an O(N^2) complexity.
...
/**
 * Official solution of Regular Expressions
 *
 * Intuition and Algorithm
 * 
 * Whenever parsing is involved, we can use regular expressions, a language for defining patterns in text.
 * 
 * Our regular expression will be "([A-Z][a-z]*)(\d*)|(\()|(\))(\d*)". Breaking this down by capture group,
 * this is:
 * 
 * - ([A-Z][a-z]*) Match an uppercase character followed by any number of lowercase characters, then ((\d*))
 ...
 *     count, top = stack.pop(), and add those entries to our current count.
 *
 * Time Complexity O(N^2), and Space Complexity O(N). The analysis is the same as Approach Recursion, as this regular expression did not look backwards when parsing.
 */
